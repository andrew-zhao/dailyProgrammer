...
Input is puzzle, image width, screen offset, time between clicks
...
"""
def solve_puzzle(puzzle, width, screen_offset, speed):
	def click(x, y):
		x = x_off + x * tile_dim
		y = y_off + y * tile_dim
	...
	# Input cells is list of tuples (x, y, value)
	def modify_list(cells):
		for x, y, value in cells:
			modify(x, y, value)

	def modify(x, y, value):
		old_value = puzzle[x][y]
		puzzle[x][y] = value
		for _ in range((value + 3 - old_value) % 3):
			click(x, y)

	def unmodify(old_puzzle):
		for x in range(dimension):
			for y in range(dimension):
				if puzzle[x][y] != old_puzzle[x][y]:
					new = puzzle[x][y]
					old = old_puzzle[x][y]
					for _ in range((old + 3 - new) % 3):
						click(x, y)
					puzzle[x][y] = old

		#puzzle = map(list(old_puzzle))
	...
	while True:
		...
		for x in range(dimension):
			for y in range(dimension):
				if puzzle[x][y] == -1:
					modified, value = empty_adjacent_check(x, y, puzzle)
					if modified:

						modify(x, y, value)
				else:
					modify_list(duo_check(x, y, puzzle))

		modify_list(row_check(puzzle))
		modify_list(column_check(puzzle))
...
Returns (modified boolean, value).
"""
...
Returns list of modified cells and color.
"""
	...
	def check_helper(x, y, puzzle, xd, yd):
		...
		if is_valid_coord(x + xd, y + yd, puzzle) and puzzle[x + xd][y + yd] == value:
			if is_valid_coord(x + (2 * xd), y + (2 * yd), puzzle) and puzzle[x + (2 * xd)][y + (2 * yd)] == -1:
				modified.append((x + (2 * xd), y + (2 * yd), opposite))
			if is_valid_coord(x - xd, y - yd, puzzle) and puzzle[x - xd][y - yd] == -1:
				modified.append((x - xd, y - yd, opposite))
		return modified

	modified = []

	if puzzle[x][y] != -1:
		modified += check_helper(x, y, puzzle, -1, 0)
		modified += check_helper(x, y, puzzle, 0, -1)
		modified += check_helper(x, y, puzzle, 0, 1)
		modified += check_helper(x, y, puzzle, 1, 0)
	return modified
		...
		if one_count == dimension/2:
			for column_index in range(dimension):
				if puzzle[row_index][column_index] == -1:
					modified.append((row_index, column_index, 0))
		elif zero_count == dimension/2:
			for column_index in range(dimension):
				if puzzle[row_index][column_index] == -1:
					modified.append((row_index, column_index, 1))
	return modified
...
This version shows each step that the algorithm makes.
